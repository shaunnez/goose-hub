...
## core/ (38 entries, 2 missing README)
...
| `workflow-routing` | **missing** | _(no README)_ |
...
## slices/ (43 entries, 1 missing README)

| Name | README | Summary |
|---|---|---|
| `acceptance-contract` | yes | Legacy acceptance-contract workflow. It turns an investigation-complete bug/chore into a durable `acceptance.contract-authored` event before legacy implementation starts. |
...
| `feature-grounding` | **missing** | _(no README)_ |
...
| `framing` | yes | Consumes the `factory:framing` state for vague fresh features. The workflow invokes `feature-frame`, appends `framedContent` to the work-item body in memory, and then continues through existing Discover Lane contracts. |
...
| `record-decision` | yes | M19.06: decision-record persistence — synchronous SQLite write with iteration + phase metadata. |
| `research` | yes | The research workflow runs a distinct research lifecycle for `type:research` work items in `factory:research-pending`. |
...
## skills/ (41 entries, 0 missing README)

| Name | README | Summary |
|---|---|---|
| `acceptance-contract` | yes | Lightweight legacy acceptance-contract authoring. It runs after investigation for legacy bug/chore work and before implementation. |
...
| `intervention-proposer` | yes | Produces structured operator decision options for durable interventions. |
...
| `research` | yes | Research answers what is true, what options exist, and what follow-up work may be needed for a `type:research` work item. |
...
## target-projects/ (3 entries, 2 missing README)

| Name | README | Summary |
|---|---|---|
| `goose-hub-self` | yes | Target-project configuration for running Factory workflows against Goose Hub itself. |
| `goose-hub-self-claude` | **missing** | _(no README)_ |
| `shift4-smartpay` | **missing** | _(no README)_ |
