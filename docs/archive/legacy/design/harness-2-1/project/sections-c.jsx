        ...
        <div style={{ display: 'grid', gridTemplateColumns: '270px 1fr', minHeight: 460 }}>
          <div style={{ borderRight: '1px solid var(--line)' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--line)' }}>
              <div className="eyebrow">Files · 12</div>
            </div>
            {D.files.map((f) => (
              <button
                key={f.path}
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  borderLeft:
                    active === f.path ? '2px solid var(--accent)' : '2px solid transparent',
                }}
              >
                <span
                  className="mono"
                  style={{
                    ...
                    fontWeight: 700,
                    width: 12,
                    color: f.status === 'new' ? 'var(--success)' : 'var(--warning)',
                  }}
                >
                  {f.status === 'new' ? 'A' : 'M'}
                </span>
                <span
                  ...
                  style={{
                    flex: 1,
                    fontSize: 11.5,
                    color: active === f.path ? 'var(--fg)' : 'var(--fg-2)',
                  }}
                >
                  {f.path.split('/').slice(-1)[0]}
                </span>
                <span className="mono tnum" style={{ fontSize: 10, color: 'var(--success)' }}>
                  +{f.add}
                </span>
                <span className="mono tnum" style={{ fontSize: 10, color: 'var(--danger)' }}>
                  −{f.del}
                </span>
              </button>
            ))}
          ...
          <div style={{ minWidth: 0, display: 'flex', flexDirection: 'column' }}>
            ...
            >
              ...
              <span className="mono tnum" style={{ fontSize: 11.5 }}>
                <span style={{ color: 'var(--success)' }}>+{file.add}</span>{' '}
                <span style={{ color: 'var(--danger)' }}>−{file.del}</span>
              </span>
            </div>
            <Diff />
          </div>
    ...
    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12.5, lineHeight: 1.55 }}>
      {lines.map((l, i) =>
        l.k === 'meta' ? (
          <div
            key={i}
            style={{ padding: '6px 16px', color: 'var(--fg-3)', background: 'var(--bg-elev-2)' }}
          >
            {l.c}
          </div>
        ) : (
          ...
          >
            <span
              className="mono tnum"
              style={{ color: 'var(--fg-4)', textAlign: 'right', paddingRight: 6 }}
            >
              {l.l}
            </span>
            <span
            ...
            <span style={{ color: 'var(--fg-3)', textAlign: 'center' }}>
              {l.k === 'add' ? '+' : l.k === 'del' ? '−' : ' '}
            </span>
            <span style={{ color: l.k === 'ctx' ? 'var(--fg-2)' : 'var(--fg)', whiteSpace: 'pre' }}>
              {l.c}
            </span>
          </div>
        ),
      ...
      <Card pad={0}>
        {suites.map((s, i) => (
          <div
            ...
            style={{
              display: 'grid',
              gridTemplateColumns: '1.5fr 1fr 100px 100px 100px',
              padding: '12px 16px',
              alignItems: 'center',
          ...
          >
            <div className="row gap-3">
              <span
                style={{
                  ...
                  height: 8,
                  borderRadius: 4,
                  background: s.s === 'pass' ? 'var(--success)' : 'var(--warning)',
                }}
              />
              <span className="mono" style={{ fontSize: 12.5 }}>
                {s.n}
              ...
              {Array.from({ length: s.tot }).map((_, j) => (
                <span
                  ...
                  style={{
                    ...
                    height: 14,
                    borderRadius: 1.5,
                    background: j < s.p ? 'var(--success)' : 'var(--line-2)',
                  }}
                />
              ))}
            </div>
            <span
              className="mono tnum"
              style={{ textAlign: 'right', fontSize: 12, color: 'var(--fg-2)' }}
            >
              {s.p}/{s.tot}
            </span>
            <span
              className="mono tnum"
              style={{ textAlign: 'right', fontSize: 12, color: 'var(--fg-3)' }}
            >
              {s.ms ? `${s.ms}ms` : '—'}
            </span>
...
function Review() {
  ...
  return (
    <Shell
      eyebrow="07 · Review"
      title="Cyra's pre-merge checklist"
      ...
      <Card pad={0}>
        {checks.map((c, i) => (
          ...
          >
            ...
            <span style={{ flex: 1, fontSize: 13, color: c.ok ? 'var(--fg)' : 'var(--fg-3)' }}>
              {c.t}
            </span>
            <button className="btn sm" style={{ opacity: c.ok ? 0.6 : 1 }}>
              <Icons.Term size={11} />
              Log
            </button>
          </div>
