...
function makeApp() {
  return new Hono().route('/api/search', searchRouter);
}
...
describe('GET /api/search', () => {
  it('returns the result payload on success', async () => {
    ...
    const res = await makeApp().request('/api/search?q=cache');
  ...
  it('passes through the limit query param when present', async () => {
    ...
    await makeApp().request('/api/search?q=foo&limit=5');
  ...
  it('defaults q to empty string when omitted', async () => {
    ...
    await makeApp().request('/api/search');
  ...
  it('returns 500 when the service surfaces an error', async () => {
    ...
    const res = await makeApp().request('/api/search?q=x');
