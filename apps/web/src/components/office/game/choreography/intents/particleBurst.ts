// Intent: particleBurst
// 8-particle burst at (worldX, worldY) via RoomLayer.particleBurstAt.
//
// Invariants:
//   - Calls only RoomLayer.particleBurstAt; no other layer.
//   - No coordinate literals; all positions come from intent.params (set by factory).
//   - Cancel stops all particles immediately.
...
export function run(intent: Intent, ctx: ChoreographyCtx): Promise<IntentResult> {
  ...
  const { promise, cancel: cancelFn } = ctx.roomLayer.particleBurstAt(
    effectId,
    worldX,
    worldY,
    durationMs,
