...
- `TransitionButton.tsx` — popover of legal next states from `GET /api/projects/:slug/issues/:id/legal-targets`; calls `POST /api/projects/:slug/issues/:id/transition`. M2.10 (#35) ships the optimistic Board move.
...
- `GatePendingBanner.tsx` — amber callout rendered between `TaskHeader` and the main content rails when the issue has an active intervention. Renders proposed decisions from the intervention row and manual fallback targets from the legal-target endpoint.
