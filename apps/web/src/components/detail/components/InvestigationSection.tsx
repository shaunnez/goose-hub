    ...
    <div className="rounded-lg border border-line bg-bg-elev overflow-hidden">
      ...
      >
        <div className="min-w-0">
          ...
          {summary != null && <div className="mt-1 text-[12px] text-fg-3">{summary}</div>}
        </div>
    ...
    <div data-testid="investigation-section" className="px-8 py-6 flex flex-col gap-5">
      ...
      {(investigate.findings.trim().length > 0 || investigate.keyFiles.length > 0) && (
        <InvestigationAccordion
          title="Findings"
          summary={`${findingsCount} item${findingsCount !== 1 ? 's' : ''} surfaced`}
          defaultOpen
        >
      ...
      {investigate.openQuestions.length > 0 && (
        <InvestigationAccordion
          ...
          summary={`${investigate.openQuestions.length} unresolved question${
            investigate.openQuestions.length !== 1 ? 's' : ''
          }`}
          defaultOpen
        >
      ...
      {humanNotes.length > 0 && (
        <InvestigationAccordion
          ...
          summary={`${humanNotes.length} note${humanNotes.length !== 1 ? 's' : ''} captured`}
          defaultOpen
        >
        ...
        <InvestigationAccordion
          ...
          summary={
            itemState === 'factory:gate-pending'
              ? 'Low-confidence investigations require explicit review before handoff'
              : 'Investigation can be promoted to dev-ready'
          }
          defaultOpen
        >
          <div
