...
export interface AcceptanceContractDetailsProps {
...
export function AcceptanceContractDetails({
  contract,
  defaultOpen = false,
}: AcceptanceContractDetailsProps) {
  if (contract == null || contract.criteria.length === 0) return null;

  return (
    <InvestigationAccordionSection
      title="Acceptance Contract"
      summary={`${SOURCE_LABEL[contract.source]} · ${contract.criteria.length} AC`}
      icon={<ClipboardCheck size={13} />}
      defaultOpen={defaultOpen}
      testId="acceptance-contract-section"
    >
