...
export type InvestigationAccordionSectionProps = {
  title: string;
  summary?: string;
  icon?: ReactNode;
  defaultOpen?: boolean;
  ...
  return (
    <div className="rounded-lg border border-[color:var(--accent)]/20 bg-bg-elev overflow-hidden">
      <button
        type="button"
        aria-expanded={open}
        data-testid={testId != null ? `${testId}-trigger` : undefined}
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          {summary != null && <span className="text-[11px] text-fg-4 truncate">{summary}</span>}
        </div>
      ...
      {open && (
        <div
          data-testid={testId != null ? `${testId}-content` : undefined}
