...
- `golden-feature-prd-revised.spec.ts` — golden feature with PRD revision: first PRD rejected via `prd-reject-btn`, grill-and-prd re-runs to produce a second PRD, then approve. Targets the current pre-ADR-0033 reject/re-grill path; should be extended once revise/decline lands.
