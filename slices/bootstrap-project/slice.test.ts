...
describe('bootstrapProject — happy path', () => {
  it('runs all four steps + opens PR + applies bootstrap label', async () => {
    ...
    expect(decoded).toContain("stateMachine: 'db'");
    expect(decoded).toContain('repositories: [');
    expect(decoded).toContain('repos: ["octo/widgets"]');
    expect(decoded).toContain('integrations: {\n      github:');
...
describe('bootstrapProject — generated config supports multiple repos safely', () => {
  it('keeps local-db as source and derives local paths from owner/repo', async () => {
    const out = renderProjectConfig({
      slug: 'platform',
      repoRef: 'org-a/api',
      repos: [
    ...
    expect(out).toContain("kind: 'local-db'");
    expect(out).toContain("stateMachine: 'db'");
    expect(out).toContain('repos: ["org-a/api","org-b/api"]');
