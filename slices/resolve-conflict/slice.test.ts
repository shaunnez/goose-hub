...
const SUCCESS_AGENT_OUTPUT: AgentResult = {
  output: {
    ...
    unresolvable: [],
    confidence: 'high',
    decisionSummaries: [{ kind: 'GREEN', summary: 'Merged both changes in foo.ts' }],
  },
  decisionSummaries: [],
  events: [],
};
  ...
  it('agent returns unresolvable files → transitions to needs-human', async () => {
    ...
    const agentOutput: AgentResult = {
      output: {
        ...
        confidence: 'low',
        decisionSummaries: [{ kind: 'GREEN', summary: 'Could not resolve hard.ts' }],
      },
      decisionSummaries: [],
      events: [],
    };
  ...
  it('agent reports low confidence → transitions to needs-human (no merge attempt)', async () => {
    ...
    const lowConfidence: AgentResult = {
      output: {
        ...
        unresolvable: [],
        confidence: 'low',
        decisionSummaries: [{ kind: 'GREEN', summary: 'Uncertain — picked PR side blindly' }],
      },
      decisionSummaries: [],
      events: [],
    };
