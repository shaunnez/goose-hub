...
description: >
  ...
  "move to M<N>", "create milestone M<N>". Updates AGENTS.md, creates GitHub milestone,
  re-labels issues.
---
...
Read `AGENTS.md` and extract the current active milestone name from this line:
```
...
Read `docs/PLAN.md` section 28 to find the **exact title** of the next milestone (e.g. "M4: Controlled Codex CLI Runtime Spike").
...
In `AGENTS.md`, find the `gh issue list` command under "Starting the next issue":
```
...
Check `.Codex/hooks/post-compact.sh` — it now derives the milestone dynamically from `schedule:current` issues, so **no manual update needed** for that file.
