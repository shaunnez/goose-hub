...
import { personaNames } from '../db/schema.js';
...
describe('selectPersona', () => {
  it('returns object with personaId and codename', () => {
    const result = selectPersona(uid(), 'developer');
  ...
  it('personaId format is projectId/role/index', () => {
    const proj = uid();
    const { personaId } = selectPersona(proj, 'developer');
    expect(personaId).toMatch(new RegExp(`^${proj}/developer/\\d+$`));
  });
